 ...
 * @param {number[]} paperData.authorIds - Array of author IDs
...
function addPaperToStore(store, paperData, filePath) {
    ...
    if (!title || !authorIds?.length || !topics?.length) {
    ...
    authorIds.forEach(authorId => {
        ...
        if (!authorExists) {
            console.log(`Checking author ${authorId}:`, store.statementsMatching(author));
...
function getPaperDetails(store, paperId) {
    ...
    const authors = authorStatements.map(stmt => ({
        id: parseInt(stmt.object.value.split('/').pop()),
...
/**
 * Gets the next available author ID from the store
 * @param {rdf.IndexedFormula} store - The RDF store
 * @returns {number} - Next available ID
 */
function getNextAuthorId(store) {
    // Get all author nodes
    const authors = store.statementsMatching(null, RDF("type"), iao("0000238"));
    
    if (authors.length === 0) return 1;
    
    // Extract IDs and find max
    const ids = authors.map(stmt => 
        parseInt(stmt.subject.value.split('/').pop())
    );
    return Math.max(...ids) + 1;
}
 ...
 * @param {string} filePath - Path to TTL file
 * @returns {Object} - Author data including assigned ID
 */
function makeNewAuthor(store, authorData, filePath) {
    const { firstName, lastName, institution } = authorData;
    
    if (!firstName || !lastName || !institution) {
        throw new Error('First name, last name, and institution are required');
    }

    // Get next available ID
    const authorId = getNextAuthorId(store);
    const author = rdf.sym(`http://example.org/Person/${authorId}`);
    ...
    store.add(author, iao("0000303"), institutionNode); // has institutional affiliation

    // Save to file
    const ttl = rdf.serialize(null, store, "http://example.org/", "text/turtle");
    fs.writeFileSync(filePath, ttl);

    console.log(`Added author ${authorId} to store`);
    return { 
        id: authorId,
        firstName,
        lastName,
